...
def apply_genetic_operators(cube, population, elite):
    ...
    for i in range(0, len(population), 2):
        ...
        if random_prob <= CROSSOVER_PROB:
            ...
            cross_offsprings = crossover_1p(cross_offsprings[0], cross_offsprings[1])
...
def genetic_algorithm(cube):
    ...
    while i < N_GENERATIONS:
        ...
        evaluate_pop(cube, population)
        # population.sort(key=get_fit_2h, key=get_fit_1h)
        ...
        if population[0]['fit'][1] == 0: break
        print("GENERATION", i, "   Curr best fit:", population[0]['fit'][1], "    Len: ", len(population), "    N moves", cube.n_moves, "    parents", len(parents))
        print([c['fit'] for c in population])
        print([m for m in population[0]['val']])
        print([m for m in population[1]['val']])
        i += 1

    print("Final pop\n", [c['fit'][1] for c in population])
    print("\nBest fit: ", population[0]['fit'][1])
    print("Best sol: ", population[0]['val'][0] + population[0]['val'][1])
